...
module.exports.execute = async function (interaction) {
    ...
    try {
        ...
        const rawData = achievementData.find(b => b.achievementID === parseInt(identifier));
        if (rawData == null) {
            await interaction.followUp({
        ...
        const infoData = getAchievementInfoEmbedData(rawData, locale);
...
module.exports.autocomplete = async function (interaction) {
    ...
    switch (focusedOption.name) {
        case 'identifier':
            choices = locale === "en-US" || focusedOption.value.trim().length <= 1 ? [...new Set(achievementNames.map(x => x[locale]))] : [...new Set(achievementNames.map(x => x[locale])), ...new Set(achievementNames.map(x => x["en-US"]))]
            break;
        default:
            focusedOption.value = "";
            choices = []
    ...
    if (focusedOption.value.trim().length <= 1) {
        await interaction.respond(choices.filter(c => !isNum(c)).sort().slice(0, 25).map(choice => {
            const nameTranslations = achievementNames.find(x => x[locale] === choice);
    ...
    const wodIds = [...new Set(names.map(n => {
        let nameTranslations = achievementNames.find(x => x[locale] === n);
        if (nameTranslations == null && locale !== "en-US") nameTranslations = achievementNames.find(x => x["en-US"] === n)
        if (nameTranslations == null) nameTranslations = achievementNames.find(x => x.raw === n);
        return nameTranslations;
    }).filter(n => n != null).map(v => v.raw))].slice(0, 25)
    await interaction.respond(wodIds.map(wodId => {
        return {name: achievementNames.find(n => n.raw === wodId)[locale], value: wodId}
    }));
}