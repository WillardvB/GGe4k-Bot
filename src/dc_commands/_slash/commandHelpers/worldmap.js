const x768 = require("../../../ingame_images/x768.json");
const {loadImage, createCanvas} = require("canvas");
const path = require('path');
const pathToImages = path.join(__dirname, '../../../ingame_images/');
...
async function loadImages() {
    images.objectBackground = await loadImage(path.join(pathToImages, x768.MapObjectBackground_Classic.url));

    images.nameTag = await loadImage(path.join(pathToImages, 'NameTag_Basic.png'));

    images.mainCastles = [null,
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level1_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level2_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level3_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level4_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level5_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level6_Classic.url)),
        await loadImage(path.join(pathToImages, x768.Castle_Mapobject_Level6_Classic.url))];

    images.outposts = [await loadImage(path.join(pathToImages, x768.Outpost_Empty.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level1.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level2.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level3.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level4.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level5.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level6.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_Level6.url))];

    images.outpostBases = [await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_2_8_0.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_2_6_0.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_2_0_8.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_2_0_6.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_8_2_0.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_6_2_0.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_8_0_2.url)),
        await loadImage(path.join(pathToImages, x768.Outpost_Mapobject_6_0_2.url))];

    images.moat = await loadImage(path.join(pathToImages, x768.Castlepart_Moat_Classic.url));

    images.background = await loadImage(path.join(pathToImages, x768.Background_Mapobject_Piece0_Classic.url));
}
        ...
        try {
            ...
            for (let mapobject of allianceCastles) {
                ...
                if (mapobject.areaType === 1) {
                    ...
                    let hkData = imagesData.mainCastles[Math.max(1, castleImageLevel)];
                    ...
                    let image = images.mainCastles[Math.max(1, castleImageLevel)];
